...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool fPrintTransactionDetail)
{
    ...
    UniValue concords(UniValue::VARR);
    BOOST_FOREACH (const CConcord& concord, blockindex->vConcord)
    {
        ...
        concords.push_back(entry);
    }
    if (concords.size())
        result.push_back(Pair("concords", concords));
